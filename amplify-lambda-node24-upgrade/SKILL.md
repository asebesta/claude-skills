...
3. **Flip the runtime and shared toolchain — centrally, once.** Do these in the orchestrator (not delegated), because they touch shared files and config that parallel fixers would clobber:
   ...
   - **Upgrading `@aws-amplify/backend` past ~1.23 swaps in a deployer that loads `backend.ts` as real ESM** — any function dir with its own `package.json` that lacks `"type": "module"` will break synth with `does not provide an export named '…'`. Fix it in the same pass; see "Deployer 2.x ESM module scoping" in amplify-config.md (includes a no-credentials local repro: `npx tsx amplify/backend.ts`).
   - **The CI build machine's Node version is separate from the Lambda runtime** — flipping `runtime: 24` does not change what Node the Amplify build runs on. Move the build to 24 too (repo `amplify.yml` `nvm install 24 && nvm use 24`, or the console live-package override) — see "Build machine Node version" in amplify-config.md.
   ...
   - Node 24 brings **npm 11**, which changes enforcement behavior, not just version numbers: e.g. `min-release-age` (supply-chain package-age gate) is silently ignored by npm 10 and only hard-enforced from **npm ≥ 11.10.0** (first bundled in Node 24.14.1). If the repo relies on `.npmrc` security settings, assert the npm version in CI rather than trusting the config to be honored.
...
5. **Build and test locally**, then deploy:
   ...
   - When validating via a **fresh feature-branch deploy**, remember it creates a brand-new stack: it can fail on account-global name collisions (CloudWatch Logs Insights query definitions, named IAM/log resources) that existing branches never re-create. Those failures are orthogonal to the runtime bump — fix or scope the names, don't misattribute them to Node 24.
